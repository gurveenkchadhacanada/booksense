...
function aiMock(a){
  if(a.name==="Meridian Health Systems")return{headline:"Active migration prep disguised as compliance review",brief:"Diana Chen's email tone has shifted from engaged to evasive — 'I'll circle back' after months of silence is a soft exit signal. Support tickets requesting API key revocation and data export are migration steps, not compliance.",assessment:"Meridian is in active churn preparation. The new CTO comes from a competitor background and is requesting data portability — this is not routine diligence. Combined with two champion departures and a 34% usage crash, every signal points to an account that has already made its decision internally.",first_action:"Call Diana Chen directly — do not email. Emails are being ignored. A direct call with a concrete save offer is the only play left before renewal in 28 days.",talking_points:["Jake Morales and Sarah Lin both left in January — acknowledge the disruption and ask who the new internal champions are","Address the data export timeout issue head-on: offer a dedicated engineering session to resolve it this week","Propose a 90-day success plan with Raj Patel — frame it as onboarding the new CTO, not saving the account"],watch_out:"Patricia Hernandez (former Meridian IT Director) is now at NovaPay Technologies — she may be influencing their competitive evaluation. Two accounts at risk from the same departure chain."};
  return{headline:`${a.name} requires ${a.usageTrend<-10?"immediate intervention":a.usageTrend>20?"expansion focus":"strategic engagement"}`,brief:`Cross-referencing CRM notes, support interactions, and recent emails reveals a ${a.npsScore>=8?"highly engaged":"potentially disengaged"} account with ${a.openTickets>0?a.openTickets+" unresolved support issues":"no open support concerns"}. ${a.daysSinceLastTouch>60?"The communication gap is a significant risk factor":"Recent touchpoints suggest the relationship is active"}.`,assessment:`${a.name} is a ${a.tier} ${a.industry} account contributing $${(a.arr/1000)|0}k ARR. ${a.usageTrend<0?"Declining usage of "+Math.abs(a.usageTrend)+"% signals potential disengagement that needs addressing before renewal.":"Usage trends are "+( a.usageTrend>20?"strongly positive":"stable")+" which "+(a.usageTrend>20?"creates expansion opportunities.":"should be reinforced with value demonstrations.")} ${a.totalTouchpoints<=2?"This account is severely under-touched relative to its value.":"Touchpoint frequency is "+(a.totalTouchpoints>=8?"healthy":"adequate")+"."}`,first_action:a.daysUntilRenewal<=30?`Schedule urgent renewal call with ${a.name} decision-maker this week.`:a.usageTrend>25?`Send expansion proposal highlighting ${6-a.adoptedProducts.length} unadopted products.`:a.daysSinceLastTouch>60?`Re-engage with a personalized usage insights report for ${a.name}.`:`Prepare QBR materials focusing on ${a.industry} benchmarks.`,talking_points:[`Current ${a.adoptedProducts.length}/6 product adoption — ${6-a.adoptedProducts.length} cross-sell opportunities remain`,`${a.industry} vertical benchmarking shows ${a.usageTrend>0?"above":"below"}-average engagement patterns`,`NPS of ${a.npsScore}/10 ${a.npsScore>=8?"positions them as an advocacy candidate":"indicates room for satisfaction improvement"}`],watch_out:`${a.daysUntilRenewal<=60?"Renewal in "+a.daysUntilRenewal+" days — ensure decision-maker alignment before contract discussions.":a.usageTrend<-10?"Usage decline may accelerate if root cause is not identified and addressed.":"Monitor for changes in stakeholder engagement or competitive signals."}`};}
async function aiDeepDive(a){try{const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1200,messages:[{role:"user",content:`Brief a CSM. ${a.name}|${a.industry}|${a.tier}|$${a.arr}|Usage:${a.usageTrend}%|Touch:${a.daysSinceLastTouch}d|Renewal:${a.daysUntilRenewal}d|Tickets:${a.openTickets}|NPS:${a.npsScore}|Touchpoints:${a.totalTouchpoints}|${a.notes}\nSignals — CRM:${a.crmNotes||""}|Support:${a.supportTranscript||""}|Email:${a.recentEmail||""}\nReturn ONLY JSON:{"headline":"<1 sentence>","brief":"<2 sentence synthesis of CRM notes, support transcript, and email signals — what do these unstructured sources collectively reveal about this account>","assessment":"<3 sentences>","first_action":"<specific>","talking_points":["<3>"],"watch_out":"<risk>"}`}]})});const d=await r.json();return JSON.parse((d.content?.[0]?.text||"").replace(/```json\s*/g,"").replace(/```/g,"").trim());}catch{return aiMock(a);}}

async function portfolioAnalysis(){try{const d=ACCOUNTS.map(a=>`ID:${a.id}|${a.name}|${a.industry}|${a.tier}|$${a.arr}|Usage:${a.usageTrend}%|Touch:${a.daysSinceLastTouch}d|Renewal:${a.daysUntilRenewal}d|Tickets:${a.openTickets}|NPS:${a.npsScore}|Touchpoints:${a.totalTouchpoints}|Notes:${a.notes}|CRM:${a.crmNotes||""}|Support:${a.supportTranscript||""}|Email:${a.recentEmail||""}`).join("\n");const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:4000,messages:[{role:"user",content:`Analyze this CSM portfolio of 20 accounts with their unstructured signals (CRM notes, support transcripts, recent emails). Consider all metrics AND the unstructured signals holistically when ranking.\n${d}\nReturn ONLY JSON:{"portfolio_summary":"<3-4 sentence narrative assessment of the entire portfolio — highlight key risks, opportunities, and overall health>","cross_account_patterns":["<identify connections between accounts — people who moved between companies, shared industries with contrasting outcomes, referral chains, competitive threats spanning multiple accounts>"],"cherry_picking_analysis":"<1-2 sentences about CSM behavioral patterns across the portfolio>","rankings":[{"id":<account ID number>,"reasoning":"<1 sentence explaining why this account is ranked at this priority position, referencing specific signals from CRM notes, support transcripts, or emails>"}]}\nThe rankings array must contain all 20 accounts ordered by priority — highest priority first. Use the unstructured signals to make nuanced ranking decisions that a rules engine would miss.`}]})});const j=await r.json();return JSON.parse((j.content?.[0]?.text||"").replace(/```json\s*/g,"").replace(/```/g,"").trim());}catch{return null;}}
